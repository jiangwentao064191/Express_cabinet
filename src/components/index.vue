          ...
          <div class="oUl_tab" style="display:block;" v-if="index==0">
            ...
            <div class="id_serviceBox">
              ...
              <div class="id_funcBox" >
                ...
                <div class="id_funcBoxL id_funcBoxL1" v-if="idty!='user'" @click="toExpressDelive" >
...
<div class="id_funcBoxLItem2">预填信息高效投件</div>
      ...
      <div class="id_footer">
        <ul id="oLi" class="card_List clearfix" ref="oLi" >
          <li :class="{'current':key==index}" v-for="(item,key) in list" :key="key" @click="switchTab(key)">
            <img :src="item.img">
            <div>{{item.name}}</div>
          </li>
        </ul>
    ...
    <div class="popup_box none">
      ...
      <ul class="company_ul">
        <li v-for="(item,key) in items" :key="key" @click="select(item.label,key)"  v-bind:class="{ company_current:key==current}">{{item.label}}</li>
      </ul>
...
export default {
  ...
  mounted() {
    ...
    this.idty=='user'?(document.title = '取快递',this.list[0].name='取快递'):document.title ='存快递';
    $(".masks").hide();
    $(".popup_box").hide();
  ...
  methods: {
    ...
    getCompanyList () {
      this.$fetch(Resource.getexpcompany).then(res=>{
        this.items = res
      }).catch(err=>{})
    },
    closed1 () {
