    ...
    <div class="sb_inputMsgs none" style="background: white;">
      ...
      <div class="sb_phonecode">
        ...
        <input
          ...
          v-model="telphone"
        >
        <div class="bg_sound" id="talk_btn"></div>
...
export default {
    ...
    confirm() {
      if (!localStorage.getItem("erCode")) {
          ...
          success: function(data) {
            ...
            _this.wx.scanQRCode({
              ...
              success: function(res) {
                ...
                _this
                  .$fetch(Resource.deliverysuccess, {
                    courierId: "2486c4c5-b224-4a3e-b987-2f2578e03643",
                    cabinetId: localStorage.getItem("erCode"),
                    boxNo:
                      _this.boxInfo.boxtype == "大格"
                        ? 4
                        : _this.boxInfo.boxtype == "中格"
                        ? 3
                        : 2,
                    boxfee: _this.boxInfo.boxfee
                      ? this.boxInfo.boxfee.substr(1)
                      : "",
                    expressId: _this.expressNumber || 12356,
                    outPhone: _this.telphone || 18317529845
                  })
                  .then(res => {
                    _this.$router.push({
                      path: "/offerDetail"
        ...
        this.$fetch(Resource.deliverysuccess, {
          ...
          cabinetId: localStorage.getItem("erCode"),
          boxNo:
            this.boxInfo.boxtype == "大格"
              ? 4
              : this.boxInfo.boxtype == "中格"
              ? 3
              : 2,
  ...
  mounted() {
    ...
    wx.ready(function() {
      ...
      let START, END, recordTimer, recordTimer2;
          ...
          success: function(res) {
            ...
            wx.translateVoice({
              ...
              complete: function(res) {
                if (res.hasOwnProperty("translateResult")) {
                  // $('#msg').text('您的手机号为：' + res.translateResult);
                  _this.telphone = res.translateResult.split("。")[0];
                } else {
                  alert("无法识别");
                }
              }
...
/* #msg { padding: 20px; background: rgba(0, 0, 0, .5); text-align: center; color:#fff;
    font-size: .24rem;
    z-index: 88;
    } */
</style>
